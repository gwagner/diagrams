import { createRef, Reference } from "@motion-canvas/core/lib/utils"
import { CloudWatch } from "valewood-components/cloud/aws/cloudwatch";
import { Enumerable, InstantiateRef } from "../../utils/setup";
import { AwsLandscapeComponentRef, BaseRef } from "./base";
import { View2D } from "@motion-canvas/2d/lib/components";
import { AWSLandscape, AWSLandscapeProps, Tags } from "../aws-landscape";

export class CloudWatchRef extends BaseRef implements AwsLandscapeComponentRef  {

    private _ref: Reference<CloudWatch>;
    public get ref(): Reference<CloudWatch> {
        return this._ref;
    }
    private set ref(value: Reference<CloudWatch>) {
        this._ref = value;
    }

    constructor(props: AWSLandscapeProps) {
        super(props)
        this._ref = createRef<CloudWatch>();

        this.tags = [Tags.Configurable, Tags.Management]
        this.addComponentRefs(this._ref)
    }

    setup()  {
        return (<CloudWatch ref={this.ref} fontSize={this.props.fontSize} tags={this.tags} />)
    }

    drawLines(view: View2D, parent: AWSLandscape): void { 

    }
}